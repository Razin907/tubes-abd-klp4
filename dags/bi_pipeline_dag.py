...
with DAG(
    ...
    task_scrape_and_extract = BashOperator.partial(
        ...
        execution_timeout=timedelta(minutes=5),  # Timeout 5 menit agar task tidak nyangkut selamanya
        max_active_tis_per_dag=2,  # Ubah ke 2 agar laptop tidak meledak jika dijalankan sendirian
    ).expand(
        bash_command=[
